...
It never fails a test and never blocks the write — the cassette is on disk either way, and the point is to put the finding in front of you while the context is still fresh, before the file is committed. Only interactions recorded *in that session* are checked, so a finding you've looked at and accepted doesn't come back every run. Under a test run the findings are held back and printed as one block after the last test rather than in the middle of the runner's progress output — every cassette the run opened, whether it came from `#[UseCassette]` or from a `VcrClient` the test built itself.
